...
import { submitOnboarding, TRAVEL_STYLE_IDS } from '@/api/onboarding';
...
export default function TravelStyleScreen() {
  ...
  const handleNext = async () => {
    if (travelStyles.length === 0 || !purpose || !location) return;
    ...
    try {
      ...
      await submitOnboarding({ purpose, location, travelStyles });
      router.replace('/home');
    } finally {
    ...
    <SafeAreaView style={styles.screen} edges={['top', 'bottom']}>
      ...
      <View style={styles.footer}>
        <PrimaryButton
          ...
          disabled={travelStyles.length === 0 || isSubmitting}
          onPress={handleNext}
        />
      </View>
    </SafeAreaView>
