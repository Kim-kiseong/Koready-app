...
// POST /api/v1/recommendation-decks — creates a fresh recommendation deck for the given
...
export async function createRecommendationDeck(
  ...
  const response = await client.post<RecommendationDeckEnvelope>('/api/v1/recommendation-decks', {
    scope,
    size,
...
// GET /api/v1/recommendation-decks/{deckId} — fetches the next page of cards for an
...
export async function fetchRecommendationDeckPage(
  ...
  const response = await client.get<RecommendationDeckEnvelope>(
    `/api/v1/recommendation-decks/${deckId}`,
    { params: cursor ? { cursor } : undefined },
  );
  return response.data.data;
}

// POST /api/v1/recommendation-decks/{deckId}/events — records a single card interaction
...
export async function recordRecommendationEvent(
  ...
  const response = await client.post<RecommendationEventEnvelope>(
    `/api/v1/recommendation-decks/${deckId}/events`,
